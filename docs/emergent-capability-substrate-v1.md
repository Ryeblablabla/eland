...
状态：扩展设计。生物个体、物质体素、五类原子动作、知识记录、授权、共同体内核与一条受限的水流机械动力链已进入运行时（当前 schema 17）；连续电力、信号、计算、多区域与高级物理层仍未实现。运行时细节与本文冲突时以 `domain/` 为准。
...
当前运行时只落地了这个设计的一条受限机械子集。河道持久化有向 `water current` 段，当前可用性由真实 Water 体素和上游连通性派生；人物必须亲自在可见、可达范围内关注具体水流段。已有 Mill 劳动造成的生产压力可以形成一个冻结源与工地几何的试建项目，但项目不会泄漏隐藏配方、时代标签或观察器目标。

已实现的物理拓扑严格限定为 `Water endpoint → WaterWheel above endpoint → horizontal DriveShaft → new Mill`。首次 commissioning 以 `progressed` 的错位故障保留项目连续性与输入 Seed，再由故障后新制造的 `DriveShaft + BronzeTool` 完成维修；维修后的真实 `Seed → Food` 作业才是项目完成证据。失流、错源、错计划、错站点或拓扑变化均在消耗输入前拒绝。这不是一般能量系统的完成声明，更不支持连续电力、远距信号、计算或信息时代。
...
新记录使用只由读者本人拥有的活跃项目及其真实技术缺口触发，来源限于本人背包和调用方已过滤的可见公共地面掉落物。v2 basis 冻结读者、项目、payload、技术和精确来源；地面来源按 `move → acquire → read → experiment` 运行，移动不计取得，只有把冻结的 exact drop 从冻结 cell + z 成功转移到本人背包才计 `acquire`，来源消失或替换时不换源。阅读只产生不高于 54 的暂定技术知识，同一真实项目输入上的实验再增加 18 并推进项目；明确直接教学仍可直接达到 60，但不冒充记录链。若同一地面来源谱系已被外部真实交付，或人物此前已经读过记录，计划可依当前真实持有与知识继续；系统不补造缺失的取得 / 阅读阶段，这类 basis 也不计完整链。
