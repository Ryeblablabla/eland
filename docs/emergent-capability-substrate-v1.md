...
状态：设计草案，不直接改变运行时 schema。

关联：[人类社会活动一千项能力地图](./human-society-capability-map-1000.md) · [最小物质—人物—动作模型](../three-body/design/minimal-emergent-human-model.md) · [月度时间模型](./monthly-time-model-v1.md)
...
当前分支的运行时观察器实际上只会正式判定三项：

- 11：采集食物；
- 20：建造住所；
- 42：开辟道路。

`three-body/README.md` 中的“65 / 1000”属于旧实现口径，不能作为当前重构分支的真实覆盖率。当前数据虽然保留关系、亲代、能力和温度等字段，但没有对应的状态转移和完整证据链。

因此不能从 3 项开始逐条添加 997 个动作。正确方向是先补齐一套最小底座，使人物只面对局部问题并执行少量动作，而千项观察器在事后识别其组合结果。
...
如果人物只响应饥饿、缺水和死亡风险，那么食物稳定后不会自然产生友谊、艺术、求知、地位竞争、宗教、教育或政治。四个偏置只决定机会成本，不直接产生行为：

- `affiliation`：亲近、照护、共同活动和归属；
- `autonomy`：避免控制、争取选择权，也可能试图控制资源；
- `recognition`：寻求胜任、声誉、地位和被承认；
- `inquiryCreation`：探索未知、试验、表达、游戏与赋予意义。

生理压力和危险由世界事实派生，不需要第五个固定偏置。现有人物卡的马斯洛五层仍可保留，但改成以下投影：

```text
生理       = 三项身体缺口
安全       = 暴露、暴力、控制与资源不确定性
归属与爱   = affiliation + 当前关系缺口
尊重       = autonomy/recognition + 社会反馈
自我实现   = inquiryCreation + 已掌握知识与可用余量
```
...
- `act`：六个固定局部作用原语，效果由目标、工具和世界规律决定；
...
真实输出由世界规则裁决，可能无效、有害或意外成功。人物通过 `attend` 比较前后事实，才把证据压缩为 `KnownTechnique`。`inquiryCreation`、迫切问题、观察他人成功和可用剩余时间只提高尝试概率，不直接解锁结果。
...
人物仍按月推进，每人每月都有非零关键决策概率。状态、疾病、物质变化、动作执行、传播、履约检查和能力观察均由本地确定性规则完成；LLM 只在关键决策时选择意图、下一原子动作或结构化表达，不会因能力地图从 3 项扩展到 1000 项而线性增加 token。

大人口下，动物、作物、病原体和无关键决策的人物使用确定性策略；重要决定才进入模型上下文。不能为了性能把共同体直接变成会瞬移资源的超级 Agent。
...
| 语言与学校 | Codebook 传播 + communicate + 模仿/练习 + 师生承诺 + 重复场所使用 + 知识变化 |
...
application/
  advance-month/  环境、身体、传播、动作、履约和事件落账
  decide-intent/  本地策略或 LLM 选择意图和下一动作
