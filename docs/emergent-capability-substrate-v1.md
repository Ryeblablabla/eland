...
状态：扩展设计。生物个体、物质体素、五类原子动作、知识记录、授权、共同体内核、一条受限的水流机械动力链与有限真实电网已进入运行时（当前 schema 17）；远距信号、计算、多区域与高级物理层仍未实现。运行时细节与本文冲突时以 `domain/` 为准。
...
当前运行时落地了受限机械子集和一条独立的有限电力子集。河道持久化有向 `water current` 段，当前可用性由真实 Water 体素和上游连通性派生；人物必须亲自在可见、可达范围内关注具体水流段。已有 Mill 劳动造成的生产压力可以形成一个冻结源与工地几何的试建项目，但项目不会泄漏隐藏配方、阶段标签或观察器目标。

已实现的机械拓扑严格限定为 `Water endpoint → WaterWheel above endpoint → horizontal DriveShaft → new Mill`。首次 commissioning 以 `progressed` 的错位故障保留项目连续性与输入 Seed，再由故障后新制造的 `DriveShaft + BronzeTool` 完成维修；维修后的真实 `Seed → Food` 作业才是项目完成证据。失流、错源、错计划、错站点或拓扑变化均在消耗输入前拒绝。这条机械链本身仍不证明电力。

独立电力链使用真实的机械发电机、绝缘铜导体与电阻负载，现场验证 source → conductor → load 拓扑、运行知识、负载结果与故障后的诊断/更换/恢复。合并后的最高阶段“现代文明（含信息能力）”要求这条有限电网当前拓扑有效并首次完成真实有用供电；再加上标定后的可比质量测量，以及能交叉核对真实项目、记录、读者/作者、知识/codebook、产物与置信度变化的他人记录实验复用。部件制造、核验、安装与拓扑已在首次供电前闭合，不额外等待没有新增展示或交互的第二次调度运行。这是为游戏缩小的可见、可操作闭环，不宣称远距信号、计算或完整工业基础已实现；后者是现代文明内部的后续成就。
...
当前耐久记录闭环以真实载体为准。知识保存项目在预先固定的场地取得或制造空白载体，由项目所有者写入目标知识；当且仅当所有者背包已有与本项目所有者、目标知识、写入动作和载体栈精确对应的已写载体时，返回场地并投放到精确地面的发布步骤才优先于尚未结束的旧搜索 / 物流。没有合格已写载体时，原制造与物流顺序不变；成功投放沿既有项目完成事实收口。带 `recordPayloadId` 的本人库存栈不能被普通 `combine / exert / expose` 作为消耗输入，规划候选与领域扣减都执行这一守恒；空白载体仍保持可写和可用于其他合法动作。

普通便携产物也不以“背包里恰好已有同材质”作为项目完成证据。当前目标材料栈必须保留至少一项属于该项目 `actionEventIds` 的来源事实，完成引用只取这部分交集；旧项目产物可以继续作为真实材料和认知来源，却不能冒充新项目成果。

新记录使用只由读者本人拥有的活跃项目及其真实技术缺口触发，来源限于本人背包和调用方已过滤的可见公共地面掉落物。V3 basis 冻结读者、项目、payload、技术和精确载体来源，但不在阅读前冻结实验动作或输入栈。地面来源按 `move → acquire → read → prepare-experiment → experiment` 运行，移动不计取得，只有把冻结的 exact drop 成功转移到本人背包才计 `acquire`，来源消失或替换时不换源。记录技术必须精确编译为项目当前缺口，但阅读前不要求实验输入全部齐备；阅读只产生不高于 54 的暂定技术知识，准备阶段逐 tick 重编译普通项目物流，真实实验才推进项目并可能跨过可靠阈值。明确直接教学仍可直接达到 60，但不冒充记录链。
...
记录使用观察器同样只读事实：原始阶段和独立违规可分别审计，但 `completeRecordUseChains` 只接纳通过读者 / 项目 / payload / codebook、精确取得、阅读理解与可靠度、实验产物与 `+18`、正确顺序和项目进度守卫的同一 basis。普通移动、无记录语义的复合对话，以及依外部真实状态跳过阶段的继续执行都不会被补写为完整链。
...
| 水流机械作业 | 本人可见水流观察 + 冻结工地假说 + WaterWheel/DriveShaft/Mill 严格拓扑 + commissioning 故障 + 新轴与工具维修 + 维修后有来源输入输出 |
