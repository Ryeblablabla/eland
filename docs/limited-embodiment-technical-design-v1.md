...
状态：2026-08-26 已实现 v1。本文以当前可执行代码为准记录已落地边界；其中标明“后续”的扩展不是已有能力。

范围：人间体素世界、单个在世人物、单玩家实时会话、一个自然月内的 15 个规划刻度，以及移动、近身交互和真实体素建造。多人化身、战斗、自由飞行、全局文明指令和工业以后内容不在 v1 范围内。

已落地的 v1 包括：第一人称相机与 15 刻 HUD、自动演化暂停、相邻移动、等待、当前合法建造 / 项目 / 转移 / 观察 / 结构化沟通候选、逐刻服务端执行、中途交还、幂等收据和 SQLite 实时会话恢复。交互层已支持等待在途普通月份后自动进入、合法移动的纯表现预测、按住方向连续续步、一个离散后续方向缓冲、忙碌时排队交还，以及 Three.js 分层失效刷新。当前化身月的 NPC 月初决策由本地 `RulePlanner` 一次生成并冻结，不发起模型请求；观察模式的自然自由文本对话也尚未并入化身行动。
...
- 不允许前端把预测提交为人物位置、材料数量或结构完成；只允许对服务端已下发的合法移动播放可丢弃的镜头表现。
...
| 交互响应 | 进入时立即使用已缓存权威人物站位切换镜头；移动请求与保留速度的临界阻尼镜头并行；`Tab` 在安全边界立即或排队交还 | 进入准备、逐刻确认与 release 收束仍是服务端互斥写操作 |
| 场景刷新 | 地形、装饰、结构选择代理和化身目标按各自真实渲染依赖失效，稳定 ID 的代理复用 | v1 逐刻 HTTP 仍返回完整 `SocietyState`；增量协议仍是后续 |
...
- 前端只能从当前 `EmbodimentView.options` 中选择合法移动，先用临时 `presentationPosition` 播放镜头；该位置不移动人物 mesh、不生成候选、不参与 raycast 合法性或下一次规则校验。
- 服务端确认后镜头就地收敛；拒绝、身体接管或返回位置不同时，镜头柔和回到新的权威站位。
...
7. 服务端排除玩家人物的自主初始选择，冻结其他人物本月需要的 tick-1 决策输入，持久化暂存月份。
8. 返回第 1 刻局部投影和合法候选；如果权威站位已变，镜头再做一次短校正。
...
1. 前端先释放 Pointer Lock 并停止续步；若当前没有命令在途，立即隐藏化身 HUD 并进入“收束本月”，否则只排队一次交还并在当前 tick 收据返回后自动执行。
2. 服务端保留已经完成的所有刻度。
3. 从 `nextTick` 开始，玩家人物恢复本地规则规划。
4. 服务器在同一释放请求内完成剩余刻度和月末提交；这一阶段对玩家表现为后台收束，但服务端写互斥仍持续到提交完成。
5. 返回普通 `GameFrame`，清除暂存月份并恢复自动演化；失败时前端恢复原化身视角供同一 `releaseId` 重试。

若玩家在 tick 命令在途时排队交还，而网络返回未知结果，前端同时保留原 `commandId` 与交还意图。此时只允许用同一行动重试对账；一旦确认成功或服务端返回已提交视图，立即自动执行已排队的交还，不允许改发其他行动。
...
`executing-tick / releasing / finalizing` 是运行时互斥临界区。客户端不得再提交并发命令；它可在 `executing-tick` 期间仅记住一个下一移动方向，待当前收据返回后再从新的权威候选中重新匹配。持久化边界只写入已完整提交命令后的 `awaiting-command`；进程不会把半执行刻度序列化。
...
- 单次轻按只提交一步并忽略键盘自动 repeat；按住时，上一权威请求返回后会从新 `EmbodimentView` 重新匹配下一步。服务端仍只有一个命令在途，另最多保留一个离散后续方向。
- 按住并转头时重新计算相机相对方向；四向扇区在 45° 边界使用约 8° 迟滞，避免细小鼠标抖动交替排队两个方向。
- 在“正在接管身体”阶段提前按住的方向会保留；进入结果成功或对账成功后才从最新服务端候选提交首步，若在返回前松键则不会误续步。
- v1 不提供跳跃、冲刺和斜向移动。
...
- 按下合法移动时，镜头立即向 `presentationPosition` 做帧率无关的临界阻尼追踪，权威请求并行执行；新锚点只改变目标并保留速度，确认命中同一锚点时不重播，纠偏也不产生过冲。
- 水平与垂直采用不同响应和限速；连续步行不会在每格边界把镜头速度清零。
- 插值和请求在途时可记录一个后续方向或维持按住方向，但服务端始终只有一个命令在途。
- 第一次 `Escape` 只释放 Pointer Lock；`Tab` 才是交还自主。Pointer Lock 被浏览器拒绝时不报业务错误，鼠标拖拽与键盘路径继续可用。
- 第一人称关闭 GTAO 和移轴、将设备像素比上限降为 1.15；准星候选以 30Hz 检测并保留 110ms 边缘黏性，不再每帧强制更新整个场景矩阵。
...
`SocietyScene3D` 继续拥有唯一 Three.js world；它只根据 `cameraMode` 切换相机控制，并把准星命中的服务端 option 目标回传给 HUD。当前实际投影包括人物、体素、地面掉落、容器、动物和遗体；`structure` 仍只存在于传输联合类型，领域 `WorldRef` 尚无对应生产者，不能把结构外观冒充成可交互动作。HUD 从服务器 option 列表中匹配目标，不在 React 中重新判断合法性。
...
底部：1 / Enter / E 当前主操作 · 1刻    F 更多（2–9）
右下：Tab 交还自主
```
...
- 给予、请求帮助和建造只有在目标与候选都存在时出现；当前原子动作是“先接近”的语义目标仍保留原人物 / 物体目标，不会因 category 为 move 被隐藏。
- `continue-intent` 始终提供全局 fallback；自身、背包等无世界目标动作也进入全局候选，不要求玩家瞄准被隐藏的本人 mesh。
- 动作细节按需展示 category、reason、risks 和 materialCost；本刻反馈固定保留一条受控人物事实，再配一条同刻世界事实。
- 行动提交后不再接受第二个并发世界行动；移动键可替换最多一个待确认方向，其他按钮等待 tick 收据与镜头收敛。
...
| 镜头与 HUD | 已完成；第一人称、Pointer Lock / 安静拖拽降级、15 刻进度、目标与全局候选、1–9 / Enter / E / F、四态反馈和忙碌时排队交还已接入 |
| 移动与等待 | 已完成；WASD 只匹配服务端返回的相邻合法 move，支持保速临界阻尼、按住连续续步、转向迟滞与一方向离散缓冲 |
| 化身性能基线 | 已完成第一阶段；场景分层失效、稳定代理复用、逐刻 clone 共享不变 committed history，`ELAND_PERF_LOG=1` 可分解 begin / step / release 热点 | HTTP 仍返回全量 society 并同步重写会话恢复根；小命令日志与 patch 协议仍是后续 |
...
- `scripts/test-player-embodiment.mjs`：候选稳定性、等待、相邻移动、建造重配、继续意图、人物不可用，以及人物结伴邀请造成协议与对方必须回应、drop 拾取改变库存、animal / remains 权威目标投影。
- `scripts/test-limited-embodiment-session.mjs`：begin 幂等、committed state 隔离、活动快照恢复、命令重放、逐刻不变历史共享、提交失败回滚、release 完成 15 刻、第 15 刻 / release 幂等和完成收据跨重启恢复。
...
- 前端从不维护第二套权威位置、库存、结构完成或文明发展事实；`presentationPosition` 只是可丢弃的镜头锚点。
...
| 每刻持久化成本过高 | v1 仍同步替换完整会话恢复根；当前先避免逐刻深拷贝不变 `world.past` 并加入分段埋点，后续再把恢复输入下沉为小命令日志 |
