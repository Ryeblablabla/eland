...
实时模型只能从本地已生成的合法候选中重选，并在创建意图前再次经过领域校验；异步模型只提出稳定语义的长期建议、玩家交互文本或创造性表达。任何模型输出都不能直接成为计划、路径、动作或世界后果。
...
模型不接收完整世界，也不提交路径数组、原子动作、持续月数或效果数值。实时重选只允许引用当次上下文的临时 option ID；这些 ID 只进入 DecisionFact 审计，不能成为跨月战略语义。
...
- reason 是本地可审计理由，不是第二套执行指令；人物口吻可在实时候选重选或异步表达任务中补充，但不能改变结构化立场与动作语义。
- methodId 必须来自本人已可靠掌握的知识，包括亲自验证或由可靠持有者明确教导所得的技术。
