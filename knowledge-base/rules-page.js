  ...
  <div class="rules-shell">
      ...
      <div class="architecture-atlas">
          ...
          <div class="architecture-stack game-stack">
            ...
            <section class="architecture-layer" data-tone="agent">
              ...
              <div class="architecture-node-grid architecture-node-grid-4">
                ...
                <div class="architecture-node"><strong>人物心智</strong><small>PersonMind · Mind intention</small></div>
                <div class="architecture-node"><strong>模型规划</strong><small>Plan · 冻结意图 → 领域步骤</small></div>
                <div class="architecture-node"><strong>本地执行</strong><small>Execution · 规则编译与实现</small></div>
        ...
        <article class="architecture-board architecture-board-wide" id="personArchitecture" aria-labelledby="personArchitectureTitle">
          <header class="architecture-board-head">
            ...
            <p>人物 Agent 由 PersonMind、Mind、Plan 与 Execution 构成：Mind 只形成意图，Plan 把冻结意图规划为领域步骤，Execution 用本地规则编译、校验并实现；模型不裁决世界事实。</p>
          </header>
          ...
          <div class="person-architecture">
            ...
            <section class="person-layer" data-tone="agent">
              <span>P2 · 语义认知简报</span><strong>person + mind + current + visible + actionSpace</strong>
              <p>精确身体数值、人格量表、坐标、内部物质枚举与重复对象先由本地确定性地翻译为自然语言；Mind 只发送人物、处境、四个心智栏目、正在承担的事项摘要、社会承诺、对话与可见事实，不发送 availableSteps、actionSpace、执行进度、下一动作类型或项目材料状态。创世先民第一次调用还会收到一次性共同抵达背景与按性格注意方向投影的宽泛初始意图，不包含动作清单。每个人物另从 baseline HEXACO 确定性取得稳定 MBTI 写作预设，请求末尾包含注意、张力、反应、表达和两条虚构说话示例；它不创造事实或能力。Plan 再单独读取冻结意图、完整 current 与全部当前规划入口；模型仍看不到 need urgency、motivation、aspiration、expectedSuccess、候选排名或隐藏失败原因。</p>
            </section>
            <div class="architecture-arrow"><span>Mind 只形成意图 ↓</span></div>
            <section class="person-layer person-layer-split" data-tone="agent">
              ...
              <div><span>P3-B · 主观方向</span><strong>Mind：utterance + goal</strong><p>Mind 只形成人物此刻想达到、维持或弄清的意图，并把它说成唯一一句原话；稳定人格预设只塑造注意、goal 与 utterance。Mind 不读取本地候选，不规划动作，也不输出执行状态或 concern 生命周期命令；创世第一次从有事实来源的背景与宽泛初始意图开始。多人同月决策时逐人单独调用 Mind，不把不同人物放进同一次模型请求。</p></div>
            </section>
            <div class="architecture-arrow"><span>Plan 读取冻结意图并形成领域 steps ↓</span></div>
            <section class="person-layer" data-tone="social">
              <span>P4 · 规划落地</span><strong>Plan → Execution → Intent / Project / HTN</strong>
              <p>Plan 的 steps 不设固定数量；Execution 只把当前入口编译为本地动作链、逐步重验并实现，不替模型改写意图或选择其他方案。</p>
            </section>
    ...
    <section class="rules-section" id="ruleTree" aria-labelledby="ruleTreeTitle">
      ...
      <div class="rule-branch-grid">
        ...
        <details class="rule-branch" data-tone="effect" open>
          ...
          <ul>
            <li><strong>六种原子动作</strong><span>move · transfer · act · attend · talk · inscribe。talk 只发出无收件人的语言，inscribe 独立处理物质记录。</span></li>
            <li><strong>统一语言波</strong><span>三体人没有独立心念通道。每个模型 MentalAct 必须带唯一 utterance / delivery，并随 DecisionFact 发出语言波；若所选步骤是社会性 talk，ActionFact 复用同一条波，不再重复广播。只有缺少模型原话的规则 talk 才进入 speech-only 表达。</span></li>
        ...
        <details class="rule-branch" data-tone="agent" open>
          ...
          <ul>
            ...
            <li><strong>共同生活</strong><span>结伴提议保存双方共知的稳定生活地点；生活区内可以各站不同格。日常取水、劳动、学习可各自行动；只在 24 个月约定的时间余量用尽、若不返回就无法累计 12 个月共同生活时，才以空闲生活槽位为目标返回。结伴、共同体身份或仅仅看见别人都不会自动追踪实时坐标；生活谈话可以因眼前人物和共同来源形成动机，但 talk 不保存听者，也不为语言强制跨地追逐。共同抵达只能支持 everyday，只有具体共同动作或已履行约定才支持 reminiscence。</span></li>
            ...
            <li><strong>生活语言</strong><span>opening 与 response 都是 optional language edge。模型决定 utterance 与 whisper / normal / call；ActionFact 不含被点名者。任何实际解码并形成 listener interpretation 的人物都可依据精确 opening 事件独立回应，response 不会过滤其他候选。open 本身不传知识、不增加 trust / bond；正式提议的 accept / reject 仍是 required，但只有相关当事人的解释事实齐备才改变协议。</span></li>
            <li><strong>解释驱动续接</strong><span>同一人物对仍记得的旧 open 来源不再作为新 handle；opening / response ActionFact 不能自我喂回成为新共同事实。response 只保存精确 referenceEventId，旧 ConversationEpisode、听者预留和共享回合状态已删除。同一 opening 可以被多个真实解释者分别回应，每条回应再次独立传播。</span></li>
            ...
            <li><strong>动物伤亡见证</strong><span>捕猎造成动物受伤或死亡时，ActionFact 按每个在世人物当刻的局部视野保存 witnessedBy。猎人记住自己的结果，真实看见的人记住同一来源的伤亡，视野外人物不知情；动物死亡会触发猎人和见证者在下一 planning tick 重新思考，但规则不直接删除旧 concern 或替人物决定新意图。</span></li>
    ...
    <section class="rules-section" id="decisionTree" aria-labelledby="decisionTreeTitle">
      ...
      <div class="decision-map" aria-label="人物详细决策树">
        ...
        <section class="decision-phase" aria-labelledby="compileTreeTitle">
          <header><span>03 · ACTION COMPILATION</span><h3 id="compileTreeTitle">最后把意图编译成可裁决、可回放的动作</h3><p>项目不是直接“完成”。规划器逐步补齐材料、路线、数量、工具与授权；每个模型决定先留下透明 thought broadcast，主动 talk 与身体动作再分别留下事实。</p></header>
          <div class="decision-tree-scroll">
            <ol class="logic-tree">
              <li>
                ...
                <ol>
                  <li><span class="edge-label edge-yes">是</span><article class="logic-node" data-kind="process"><span>C2-A</span><h4>生成直接动作</h4><p>从 move / transfer / act / attend / talk / inscribe 中选择一步；act 再指定物质操作，inscribe 消耗真实可写载体。talk 没有收件人，执行时才按最低材质路径结算每个人的信号与解释。</p></article></li>
                ...
                <ol>
                  <li><span class="edge-label">已生成动作</span>
                    ...
                    <ol>
                      ...
                      <li><span class="edge-label edge-yes">是</span>
                        <article class="logic-node" data-kind="action"><span>COMMIT</span><h4>提交可回放事实</h4><p>DecisionFact 保存本次唯一 utterance 广播；社会性 talk 的 ActionFact 引用并复用同一波，同时保存逐人解释。其他 ActionFact 保存 talk / inscribe 与身体动作的路径、消耗、产物和成功 / 阻塞。</p></article>
                        <ol>
    ...
    <section class="rules-section" id="authorityBoundary" aria-labelledby="authorityBoundaryTitle">
      ...
      <aside class="model-sidecar">
        ...
        <div><span>MODEL MIND LOOP</span><strong>模型在月初和月内认知事件后形成 MentalAct，但不能绕过本地规则</strong><p>真实沟通、观察、combine / exert / expose 结果、失败，以及本人完成或亲眼见证的动物死亡，会把相关人物加入下一 tick 前的批量心智请求；每人每月最多两次。`mental-act-context-v3` 直接说明月份、身体处境、人物倾向、四类心智内容、语义步骤和可操作对象，不发送内部小数、坐标、评分、重复库存或物质枚举。输出从下一 tick 生效。协议错误或网络失败只退回本地合法链，已经执行的事实不回滚，也不重新调用模型回放。</p></div>
      </aside>
    </section>
    ...
    <section class="rules-section source-section" id="ruleSources" aria-labelledby="ruleSourcesTitle">
      ...
      <div class="source-grid">
        ...
        <article><strong>模型重选</strong><code>domain/action-option-semantics.ts · application/rule-planner.ts · application/model-decision/* · server/backend-decider.ts · model-decision-gateway.ts</code><span>模型模式拥有 optional talk，自愿社交失败不由本地补造；required / commitment / survival / physical 仍本地执行。Mind 使用 `mind-intention-context-v2` 只形成 `utterance + goal`，不读取步骤或输出执行状态；每个人物分别调用，并在请求末尾读取其稳定 MBTI 写作预设。每项合法意图都进入 `model-plan-context-v1`。Plan 读取冻结意图、全部当前入口与动作空间，输出不限固定数量的领域 steps；Execution 合并为兼容 Decision / Intent 后最终重验</span></article>
        ...
        <article><strong>动作裁决</strong><code>domain/action-executor.ts</code><span>六种原子动作与九种物质操作的后果</span></article>
