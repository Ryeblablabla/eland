...
const TARGET_ID = 'candidate-gen5-social-v6-cont-20260821-s20260815-m948-y200-r1';
const TARGET_LABEL = 'candidate-gen5-social-v6-cont-20260821 seed=20260815 from=948 through=2400';
const SOURCE_MONTH = 948;
const SOURCE_ENDPOINT = 1200;
const TARGET_ENDPOINT = 2400;
const V5_MANIFEST = '/Users/wangyu.rye/Desktop/eland/three-body/data/experiments/candidate-gen5-social-v5f-cont-20260821-s20260815-m948-y200-r1-continuation-manifest.json';
  ...
  const walk = (left, right, currentPath) => {
    ...
    if (Array.isArray(left) && Array.isArray(right)) {
      if (left.length !== right.length) {
        differences.push({ path: `${currentPath}.length`, kind: 'value', source: left.length, target: right.length });
      }
...
const manifest = JSON.parse(await readFile(V5_MANIFEST, 'utf8'));
const allowedBackfills = new Map(manifest.stateDifferenceAudit.historicalIntentAgreementIdBackfills.map((item) => [
  item.path,
...
assert.equal(
  ...
  await realpath('/tmp/eland-gen5-v6.txS4lV/three-body'),
);
...
const bundleDirectory = await mkdtemp(path.join(tmpdir(), 'eland-v6-continuation-create-'));
const scratchDirectory = await mkdtemp(path.join(tmpdir(), 'eland-v6-continuation-preflight-'));
...
try {
  ...
  // Exercise the exact official create/migration path in an isolated SQLite store first.
  ...
  await scratchStore.create({ id: 'v6-continuation-preflight', label: 'v6 continuation preflight', state: scratchInput });
  const scratch = await scratchStore.load('v6-continuation-preflight');
  ...
  assert.equal(differences.length, 27, `unexpected V6 create normalization difference count: ${differences.length}`);
  for (const difference of differences) {
    const allowed = allowedBackfills.get(difference.path);
    assert.ok(allowed, `unexpected V6 create normalization path: ${difference.path}`);
  ...
  // Reassert absence immediately before the one authoritative create call.
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    historicalProvenanceNormalizations: differences,
    otherDifferences: 0,
  }, null, 2)}\n`);
} finally {
  scratchStore?.close();
