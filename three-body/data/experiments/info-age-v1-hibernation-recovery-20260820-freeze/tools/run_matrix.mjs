...
async function requestResult(baseUrl, path, init) {
...
async function jsonRequest(baseUrl, path, init, attempts = 5) {
  let lastError;
...
async function optionalJsonRequest(baseUrl, path) {
  try {
...
async function main() {
  ...
  const chaosIntensity = Number(args['chaos-intensity'] ?? 0);
  if (!Number.isFinite(chaosIntensity) || chaosIntensity < 0 || chaosIntensity > 10) throw new Error('chaos-intensity must be between 0 and 10');
  ...
  for (const plan of plans) {
    let evolution = await optionalJsonRequest(baseUrl, `/api/runs/${plan.runId}/evolution`);
    if (evolution) process.stderr.write(`[matrix] resume ${plan.runId} from ${evolution.reachedMonth}/${plan.months} ${evolution.status}\n`);
    else {
      process.stderr.write(`[matrix] create ${plan.runId}\n`);
      await jsonRequest(baseUrl, '/api/runs', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          id: plan.runId,
          label: `${prefix} seed=${plan.seed} years=${plan.years} repeat=${plan.repeat}`,
          seed: plan.seed,
          config: {
            civilizationNo,
            chaosIntensity,
            climateBias,
            endpoint: { kind: 'months', value: plan.months },
          },
        }),
      });
      await jsonRequest(baseUrl, `/api/runs/${plan.runId}/evolve`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
    ...
    for (;;) {
      evolution = await jsonRequest(baseUrl, `/api/runs/${plan.runId}/evolution`);
      process.stderr.write(`[matrix] ${plan.runId} ${evolution.reachedMonth}/${plan.months} ${evolution.status}\n`);
      if (evolution.status === 'completed') break;
      if (evolution.status === 'failed') throw new Error(`${plan.runId} failed: ${evolution.error ?? 'unknown evolution error'}`);
  ...
  const result = {
    ...
    metricVersions: {
      behaviorObserver: 'causal-person-month-v21',
