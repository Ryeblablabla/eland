...
const OBSERVER_VERSION = 'hibernation-recovery-chain-audit-v4';
...
function strictMetricResult(value, supportedObservations, unsupportedObservations, options = {}) {
  const result = metricResult(value, supportedObservations, unsupportedObservations, options);
...
function auditRun(matrixRun, persisted) {
  ...
  const safeExitSignals = signals.filter((signal) => signal.fullExit && recoveryState(signal).status === 'safe');
  stableReplayUnsupportedActions += signals.filter((signal) => signal.fullExit
    && recoveryState(signal).status === 'unsupported').length;
  for (const item of indexedEvents.filter((candidate) => completedAct(candidate, 'dehydrate'))) {
    ...
    const priorExit = [...safeExitSignals]
    ...
    if (!['stable', 'chaotic'].includes(epoch) || !intent) {
      stableReplayUnsupportedActions += 1;
      continue;
    }
    ...
    const oldEntryIntent = createdAtMonth !== null && exitAtMonth !== null && createdAtMonth <= exitAtMonth;
  ...
  if (!validClosedSuspensions.length) postRecoveryResponseUnsupportedWindows += 1;
    ...
    stableEpochDehydrateReplays: strictMetricResult(
      ...
      stableReplayUnsupportedActions,
      {
        ...
        reasonCodes: stableReplayUnsupportedActions
          ? ['post-exit-dehydrate-missing-explicit-epoch-or-resolvable-intent']
          : [],
...
async function main() {
  ...
  if (!matrixArgument) {
    throw new Error('usage: node scripts/audit-hibernation-recovery-chain.mjs <matrix.json> [output.json]');
  }
