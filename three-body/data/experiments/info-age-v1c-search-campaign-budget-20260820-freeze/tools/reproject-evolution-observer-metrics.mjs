...
function interruptIntentMetrics(state) {
  ...
  return {
    ...
    interruptUnresolvedTerminalChildren: children.filter((child) => (
...
function projectPressureMetrics(state) {
  ...
  const auditSource = (project, sourceId) => {
    ...
    if (event.kind === 'action' && event.action?.kind === 'act'
      && event.action.operation === 'hunt' && event.who !== project.ownerId) crossOwnerHuntSources.add(sourceKey);
    if (event.kind === 'environment' && event.change === 'animal'
      && (event.diff?.process !== 'attack-human' || event.diff?.victimId !== project.ownerId)) nonThreatAnimalSources.add(sourceKey);
  };
  for (const project of projects) {
    ...
    for (const sourceId of Array.isArray(project?.triggerFactIds) ? project.triggerFactIds : []) auditSource(project, sourceId);
  }
  return {
...
function projectProgressMetrics(state) {
  ...
  const terminalMonth = (project) => project.status === 'completed'
    ...
    : project.status === 'blocked' ? project.blockedAtMonth
  ...
  for (const project of projects) {
    ...
    for (const item of evidence) {
      ...
      if (item.kind === 'logistics-advance') {
        logisticsAdvance += 1;
        if (!Number.isFinite(item.distanceBefore) || !Number.isFinite(item.distanceAfter)
    ...
    if (project.status === 'blocked' && typeof project.blockedReason === 'string'
      && project.blockedReason.startsWith('复核期内') && Number.isFinite(project.blockedAtMonth)
...
function searchCampaignMetrics(state) {
  ...
  for (const project of projects) {
    const campaigns = Array.isArray(project?.searchCampaigns)
      ? project.searchCampaigns.filter((campaign) => campaign && typeof campaign === 'object') : [];
    const searches = (Array.isArray(project?.logisticsEpisodes) ? project.logisticsEpisodes : [])
      .filter((episode) => episode?.kind === 'search');
    ...
    for (const campaign of campaigns) {
      ...
      for (const episode of searches.filter((candidate) => candidate.searchCampaignId === campaign.id)) {
        ...
        if (!attemptedSet.has(`${episode.target?.cellId}:${episode.target?.z}`)) targetNotAttempted += 1;
      }
    }
  }
  return {
...
function observe(state) {
  ...
  const projectIntentIds = new Set((Array.isArray(state?.intents) ? state.intents : []).flatMap((intent) => (
    typeof intent?.id === 'string' && typeof intent?.projectId === 'string' && intent.projectId.length
      ? [intent.id]
      : []
  )));
  ...
  for (const event of events) {
    if (event?.kind === 'decision' && typeof event.decision?.reason === 'string' && event.decision.reason.startsWith('生活复核：')) {
      ...
      if (event.decision.kind === 'revise' && projectIntentIds.has(event.decision.intentId)) projectLifeReviewDecisions += 1;
      continue;
    }
    if (event?.kind !== 'action') continue;
    if (typeof event.intentId === 'string' && typeof event.who === 'string' && Number.isFinite(event.atMonth)) {
  ...
  return {
    ...
    projectActionMonthShare: actionPersonMonths.size
      ...
      : 0,
    reproductionOffers,
    reproductionAcceptances: reproductionAcceptanceRefs.filter((id) => reproductionOfferIds.has(id)).length,
    ...
    ...interruptions,
    ...projectPressureMetrics(state),
    ...projectProgressMetrics(state),
    ...searchCampaignMetrics(state),
    ...projectMaterialDemandMetrics(state),
    ...projectSourceMetrics(state),
    ...shelterAdaptationMetrics(state),
  };
}

const matrixPath = resolve(matrixArgument);
...
const metricNames = [
  ...
  'reproductionOffers', 'reproductionAcceptances', 'reproductionAttempts', 'reproductionConceptions',
  ...
  'lifeReviewDecisions', 'projectLifeReviewDecisions',
  ...
  'interruptReturnLatencyMeanMonths', 'interruptReturnLatencyMaxMonths',
  ...
  'orphanSuspendedProjectIntents',
  ...PROJECT_PRESSURE_METRIC_NAMES,
  ...PROJECT_PROGRESS_METRIC_NAMES,
  ...SEARCH_CAMPAIGN_METRIC_NAMES,
  ...MATERIAL_DEMAND_METRIC_NAMES,
  ...PROJECT_SOURCE_METRIC_NAMES,
  ...SHELTER_ADAPTATION_METRIC_NAMES,
];
const aggregates = (matrix.aggregates ?? []).map((aggregate) => {
  const group = runs.filter((run) => run.years === aggregate.years);
  return {
    ...aggregate,
    ...Object.fromEntries(metricNames.map((name) => [name, numericSummary(group.map((run) => (
      run[name] === null || run[name] === undefined ? Number.NaN : Number(run[name])
    )))])),
  };
});
const result = {
  ...
  metricVersions: { ...(matrix.metricVersions ?? {}), behaviorObserver: 'causal-person-month-v14' },
...
process.stdout.write(`reprojected observer metrics for ${runs.length} runs to ${outputPath}\n`);
