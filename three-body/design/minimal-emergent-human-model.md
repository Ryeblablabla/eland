...
状态：本文核心模型已成为当前运行时主体（schema 14 起；当前 schema 17）。个别字段名与接口形状以 `src/game/eland/domain/` 为准；千项能力地图要求的能量/信号、多区域等扩展统一见 [《千项能力的最小涌现底座 v1》](../../docs/emergent-capability-substrate-v1.md)。人物规划频率与模型边界见[《规则优先人物架构 v1》](../../docs/rule-first-agent-architecture-v1.md)。
...
```ts
// 设计形状；运行时字段名以 domain/person.ts 为准
// （如 lifespanMonths 对应本文 longevityBaselineMonths，parents 对应 geneticParents）
...
type SourceOperation =
  ...
  | 'hunt'       // 追踪并捕杀动物实体
  | 'dehydrate'  // 主动进入低代谢脱水休眠（乱纪元生存）
  | 'rehydrate'; // 在水源或同伴帮助下重新水化
```
...
目标可以持续很多个月；每月的 15 个规划刻度继续执行、编译或修复下一原子动作。稳定目标不会每个刻度改换；目标完成、动作受阻、生存属性进入危险区或出现重要新事实时，本地规划器立即重新选择手段。历史意图不永久堆在活动状态中，完成后只留事件。
...
状态进入/退出、技术结算、信任账本、交易识别、意图选择、下一动作编译和制度观察都由本地确定性规则完成。实时模型最多从当次上下文的合法 option ID 中重选并补充措辞，领域层仍重新组合和校验意图；模型不能选择下一原子动作。长期目标、玩家交互内容、创造性命题或叙事表达继续通过受约束任务进入。

## 五、最小食物闭环

旧的属性层文明曾在约十年后统一因食物耗尽而灭绝。改格子模型的第一价值，就是让食物生产来自物质变化（种植—收获闭环已在运行时成立，并被观察器识别为第 32 项）：
...
application/
  advance-month  环境变化、15 个规划刻度、身体消耗、事件落账
  plan-tick      本地规则选择目标、编译/预演/修复并执行合法动作
  model-task     异步长期战略、交互、创造性与叙事建议
...
- 模型不能选择临时原子动作 ID；长期建议必须被本地规划器编译和验证，模型不能创建物质或直接声明结果。
