...
规则模拟先提交权威 `SimulationState`，模型只为已经发生的关键对话、人物记忆和历史片段补充自然表达。模型任务不进入月度事务，也不修改 `SimulationState`。

每个 run 的增强队列和结果保存在 SQLite `artifacts` 表，并通过共享 `chunks` 表引用 V8 serialize + Brotli 内容块：

```text
run_id + kind=narrative-enhancements -> artifacts -> chunks
```
...
筛选结果会在调用模型前先做确定性归并：去掉同阶段重复的冷热“加重”；疾病事件只在明确带有前后阶段且未变化时才去掉，避免把治疗后复发误判为重复。同一周期的对称约定会合并，项目完成会覆盖同源的原子动作。归并条目保留所有来源事件 id；交换、援助等有方向的约定不按人物名单简单合并。
...
第 0 月的共同抵达事实和文明结局由服务端直接写入史册，不再由前端临时补句。文明毁灭时，结局条目会取代同源的个体死亡条目，避免同一结局重复叙述。
...
- `dialogue`：只选已完成的 `talk` 事件；接受或拒绝会同时引用被回应的提议事件。
...
SQLite 中的权威运行状态与检查点不读取增强结果。回放和继续演化也不会重新调用模型或消费这些文本；增强内容只能由投影层按来源事件读取。
