...
const projectOptionsBundlePath = path.join(temporaryDirectory, 'project-options.mjs');
const interactionRulesBundlePath = path.join(temporaryDirectory, 'interaction-rules.mjs');
...
try {
  ...
  execFileSync(path.resolve('node_modules/.bin/esbuild'), [
    'src/game/eland/application/project-options.ts', '--bundle', '--platform=node', '--format=esm', `--outfile=${projectOptionsBundlePath}`,
  ], { stdio: 'pipe' });
  execFileSync(path.resolve('node_modules/.bin/esbuild'), [
    'src/game/eland/domain/interaction-rules.ts', '--bundle', '--platform=node', '--format=esm', `--outfile=${interactionRulesBundlePath}`,
  ], { stdio: 'pipe' });
  ...
  const { recordProjectAction, recompileProjectNextAction } = await import(`${pathToFileURL(projectOptionsBundlePath).href}?test=${Date.now()}`);
  const { inventoryCombinationForOutput, inventoryCombinationTechniqueId } = await import(`${pathToFileURL(interactionRulesBundlePath).href}?test=${Date.now()}`);
  ...
  const isolate = (state) => {
    ...
    const leatherClothingRule = inventoryCombinationForOutput(33);
    assert.ok(leatherClothingRule, '物流夹具需要一条可知的兽皮衣配方');
    const techniqueId = inventoryCombinationTechniqueId(leatherClothingRule);
    actor.knowledge = [{
      id: techniqueId,
      kind: 'technique',
      summary: '已核验的兽皮衣制作方法',
      confidence: 100,
      learnedAtMonth: 0,
      sourceEventIds: [`source:${techniqueId}`],
    }];
  ...
  assert.ok(firstSearchOption, '缺料项目应先暴露一项基于人物已知配方的合法项目行动');
  assert.equal(searchProject.logisticsEpisodes.length, 0, '只预览候选不得写入权威项目物流状态');
  recompileProjectNextAction(searchState, searcher, searchProject.id);
  const searchEpisode = searchProject.logisticsEpisodes[0];
  assert.ok(searchEpisode?.kind === 'search' && searchEpisode.status === 'active', `实际采用项目后必须建立持久局部搜索事件：${JSON.stringify({ option: firstSearchOption?.id, episodes: searchProject.logisticsEpisodes })}`);
  ...
  const budgetState = createInitialState(492, { endpoint: { kind: 'months', value: 12 }, chaosIntensity: 0 });
  const budgetSearcher = isolate(budgetState);
  const budgetProject = projectFor(budgetSearcher.id, 'test-search-campaign-budget-project');
  budgetProject.reviewAtMonth = 100;
  budgetState.projects.push(budgetProject);
  budgetState.world.drops = [];
  recompileProjectNextAction(budgetState, budgetSearcher, budgetProject.id);
  const firstBudgetEpisode = budgetProject.logisticsEpisodes[0];
  const budgetCampaign = budgetProject.searchCampaigns?.[0];
  assert.ok(firstBudgetEpisode?.kind === 'search' && budgetCampaign?.status === 'active', '预算夹具必须先建立活跃搜索 campaign');
  firstBudgetEpisode.actionBudget = 16;
  const searchFact = (id, episode, cause = 'intent') => ({
    id,
    kind: 'action',
    atMonth: 1,
    orderInMonth: 0,
    cellId: budgetSearcher.position.cellId,
    actionTick: 1,
    who: budgetSearcher.id,
    cause,
    action: { kind: 'move', toCellId: episode.target.cellId, toZ: episode.target.z },
    fromCellId: budgetSearcher.position.cellId,
    toCellId: budgetSearcher.position.cellId,
    fromZ: budgetSearcher.position.z,
    toZ: budgetSearcher.position.z,
    pathSegment: [],
    status: 'progressed',
    result: '测试搜索动作已提交',
    diff: {},
  ...
  const commitFact = (fact) => {
    recordProjectAction(budgetState, budgetProject.id, fact);
    budgetState.world.past.push(fact);
  };
  for (let index = 0; index < 10; index += 1) commitFact(searchFact(`search-first-${index}`, firstBudgetEpisode));
  firstBudgetEpisode.status = 'fulfilled';
  firstBudgetEpisode.endedAt = 1;
  firstBudgetEpisode.endingReason = 'search-target-reached';
  delete budgetProject.activeLogisticsEpisodeId;
  recompileProjectNextAction(budgetState, budgetSearcher, budgetProject.id);
  const secondBudgetEpisode = budgetProject.logisticsEpisodes[1];
  assert.ok(secondBudgetEpisode?.kind === 'search'
    && secondBudgetEpisode.searchCampaignId === firstBudgetEpisode.searchCampaignId,
  '新搜索 episode 必须继续消耗同一 campaign 的共享预算');
  secondBudgetEpisode.actionBudget = 16;
  for (let index = 0; index < 5; index += 1) commitFact(searchFact(`search-second-${index}`, secondBudgetEpisode));
  commitFact(searchFact('survival-move-does-not-count', secondBudgetEpisode, 'survival-reflex'));
  const sixteenthSearchAction = recompileProjectNextAction(budgetState, budgetSearcher, budgetProject.id);
  assert.equal(sixteenthSearchAction?.kind, 'move', '15 个项目搜索动作后仍应允许编译第 16 步，生存反射移动不计入预算');
  commitFact(searchFact('search-sixteenth', secondBudgetEpisode));
  assert.equal(budgetCampaign.status, 'exhausted', '跨 episode 提交第 16 个搜索动作时 campaign 必须立即耗尽');
  assert.equal(typeof budgetCampaign.closedAt, 'number', '耗尽的搜索 campaign 必须记录关闭月份');
  assert.equal(secondBudgetEpisode.status, 'exhausted', '第 16 步必须终止当前搜索 episode');
  ...
  assert.equal(budgetProject.status, 'active', '当前有限搜索刚穷尽时，原项目应保留到有界复核而非同月永久终止');
  ...
  assert.equal(postBudgetSourceAction?.kind, 'transfer', '复核前出现的真实可见来源应让同一项目恢复行动');
  assert.equal(budgetProject.logisticsEpisodes.at(-1)?.kind, 'drop', '恢复项目应锁定真实 drop，而不是重开旧 search campaign');
  assert.equal(budgetProject.searchCampaigns.length, 1, '新证据不得追加相同 basis 的搜索 campaign');
  ...
  assert.ok(dropOption, '可见缺料来源应产生合法项目行动');
  assert.equal(dropProject.logisticsEpisodes.length, 0, '候选预览不得提前锁定可见物资');
  recompileProjectNextAction(dropState, collector, dropProject.id);
