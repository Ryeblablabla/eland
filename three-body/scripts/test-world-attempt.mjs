...
try {
  ...
  writeFileSync(config, JSON.stringify({ schemaVersion: 1, endpoints: { fixture: {
  ...
  const input = { intentionChange: { goal, orientation: 'construction', horizon: 'ongoing' },
    declaration: { utterance: words, delivery: 'normal', speechIntent: { kind: 'proposal', proposalKind: 'joint-action',
      counterpartHandles: [partnerHandle], terms: { summary: '一起扶稳并重排已有木料' } } },
  ...
  assert.equal(result.body.providerRequests, 2);
  ...
  const worldContext = JSON.parse(contextMessage(sent[1]).content);
  ...
  assert(!JSON.stringify(sent[1].format).includes('completionReview'));
  assert(!JSON.stringify(sent[1].format).includes('resumeIntentHandle'));
  ...
  assert.equal(speechContext.schemaVersion, 'world-speech-context-v1');
  ...
  replies = [{ attempt: { kind: 'continue' }, declaration: {
    utterance: replyWords, delivery: 'normal', speechIntent: { kind: 'request-information' },
  } }];
  const independentReply = await api.handleDecide({ contexts: [partnerRequest] }, 'fixture');
  assert.equal(independentReply.body.providerRequests, 1);
  api.commitDecision(state, partner, partnerContext, independentReply.body.decisions[0], true, 1, events, tick++);
  assert(state.agreements.every((agreement) => !agreement.acceptedByPersonIds.includes(partner.id)));
  context = domainContext(); request = api.buildDecisionRequestContext(context);
  const exactWords = '我们先说清楚各自愿意交换什么。';
  replies = [{ attempt: { kind: 'speak', description: '把本人选好的原话说出来' }, declaration: {
    utterance: exactWords, delivery: 'normal', speechIntent: { kind: 'expression' },
  } }];
  const exactSpeech = await api.handleDecide({ contexts: [request] }, 'fixture');
  assert.equal(exactSpeech.body.providerRequests, 1, 'already selected words are not compiled or repeated by World');
