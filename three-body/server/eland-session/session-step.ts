...
export function createSessionBeginning(input: {
  ...
  controller.ownedState().civilization.externalEraRegime = {
    sinceMonth: 0,
    candidateEpoch: null,
    candidateSinceMonth: 0,
...
interface SessionStepHost {
  ...
  /** Verified candidates are rechecked by the speech service; branch-local and read-only. */
  priorSpeechLines(): SpeechLineView[];
...
export class SessionStepCoordinator {
  ...
  private async advanceStep(options: ElandStepOptions): Promise<GameFrame | null> {
    ...
    try {
      ...
      // Capture once before the month starts. Lines generated by this step are
      // unavailable to its own decisions and become context only after record().
      const priorSpeechLines = this.host.priorSpeechLines();
      ...
      let speechUsage: TokenUsage = { inputTokens: 0, outputTokens: 0 };
      let speechProviderRequests = 0;
      try {
        const speech = await speechPromise;
        speechLines = speech.lines;
        speechUsage = speech.usage;
        speechProviderRequests = speech.providerRequests;
      ...
      if (modelLedger?.atMonth === state.clock.elapsedMonths && speechProviderRequests > 0) {
        ...
        if (speechUsage.cacheHitInputTokens !== undefined) {
          modelLedger.cacheHitInputTokens = (modelLedger.cacheHitInputTokens ?? 0) + speechUsage.cacheHitInputTokens;
        }
        if (speechUsage.cacheMissInputTokens !== undefined) {
          modelLedger.cacheMissInputTokens = (modelLedger.cacheMissInputTokens ?? 0) + speechUsage.cacheMissInputTokens;
        }
        modelLedger.providerRequests = (modelLedger.providerRequests ?? 0) + speechProviderRequests;
        modelLedger.speechInputTokens = speechUsage.inputTokens;
        modelLedger.speechOutputTokens = speechUsage.outputTokens;
        modelLedger.speechProviderRequests = speechProviderRequests;
      }
      for (const line of speechLines) {
        ...
        writeLanguageMemory(state, {
          ...
          ...(line.dialogueMove ? { dialogueMove: line.dialogueMove } : {}),
          ...(line.disposition ? { disposition: line.disposition } : {}),
          ...(line.replyToSpeechLineId ? { replyToSpeechLineId: line.replyToSpeechLineId } : {}),
        });
      }
