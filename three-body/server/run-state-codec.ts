...
import { createRunStateBoundedDecoder } from './run-state-bounded-decoder';
...
const runStateBoundedDecoder = createRunStateBoundedDecoder({
...
/** Materialize exact, selected event bodies from one verified history root. */
export function materializeVerifiedRunHistoryPinnedEvents(
  root: RunStateRootMetadata,
  readChunk: (hash: string) => RunStateChunk,
  absoluteIndexes: readonly number[],
): RunStatePinnedEvent[] {
  return runStateBoundedDecoder.materializeVerifiedRunHistoryPinnedEvents(
    root,
    readChunk,
    absoluteIndexes,
  );
...
/** Decode one state while retaining only a bounded authoritative history window. */
export async function decodeSegmentedRunStateBounded(
  rootChunk: RunStateChunk,
  readChunk: (hash: string) => RunStateChunk,
  options: RunStateBoundedDecodeOptions,
): Promise<DecodedBoundedRunState> {
  return runStateBoundedDecoder.decodeSegmentedRunStateBounded(
    rootChunk,
    readChunk,
    options,
  );
...
/** Decode the closed schema-3 gameplay continuation profile. */
export async function decodeSegmentedRunStateGameplayBounded(
  ...
  return runStateBoundedDecoder.decodeSegmentedRunStateGameplayBounded(
    rootChunkInput,
    readChunk,
    options,
  );
