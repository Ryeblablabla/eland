...
- `application/projects/project-step-compiler.ts`：保留项目步骤公共门面；`steps/construction.ts`、`cultivation.ts`、`care.ts` 与 `known-material-production.ts` 分别承接遮蔽构筑、定居耕作、治疗和共享已知物质生产步骤，避免总编译器继续按功能横向膨胀。质量比较缺少仪器时只提出“两个相同刚性构件 + 柔性悬挂件”的对称装置假说；已有仪器后只提出“硬质稳定实体 + 柔性标记”的参考物假说。`project-hypotheses.ts` 只按本人当前实体的固体 / 金属 / 纤维、粗硬度、粗质量和来源角色排列二或三份实体的有限试验，不查询目标产物的隐藏配方；已经取得的仪器、参考物和已写载体不会被当作普通耗材。
