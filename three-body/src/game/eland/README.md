...
- `domain/action.ts`：六种原子动作（`move / transfer / act / attend / talk / inscribe`）、十种 `SourceOperation`、`WorldRef` 与 `Intent` 类型。talk 只有说话者主观含义和可选音量，没有收件人、受众或声道；inscribe 独立承担物质记录。
...
- `domain/mental-act.ts`：模型可持久化的主观心智动作，包含必填 `utterance / delivery`、目标、策略、假说、预期观察和来源；它证明“人物形成并发出了这句话”，不证明假说或结果为真。三体人没有独立心念通道，每个模型 DecisionFact 都形成唯一语言广播，旁人只记住各自的噪声版本。自由策略只留在 DecisionFact 中供审计，不会作为下一轮 Mind 的证据；只有本地已选中的合法 durable option 或重验通过的 opaque probe 才能成为 agenda 的可执行方法。
...
- `server/model-decision-gateway.ts`：模型决策分成两次职责明确的调用。Mind 只输出 goal 与唯一 utterance，不输出执行状态；多人同月决策时也逐人单独调用 Mind，避免人物上下文和人格示例在同一请求中串扰。每项合法意图都进入 Plan；多人 Plan 仍可按 agentHandle 批量调用。Plan 输出不限固定数量的领域规划 steps，并选择当前交给 Execution 的 firstStep / continuation / experiment 入口。网关合并两阶段结果为兼容 `Decision / MentalAct`，application 继续按最新局部事实把入口编译成 Intent、逐 tick 动作链与 ActionFact。协议错误可重试但不进入人物历史；材料机理、远处障碍、设施效果和他人选择仍必须经真实行动后反馈。`MODEL_CHARACTER_AGENDA_MODE=off` 仍可关闭 concern 更新。
