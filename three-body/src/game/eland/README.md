...
人物页主动对话由 `server/agent-interaction-gateway.ts` 调用 `interaction` 模型。玩家不需要区分普通对话与建议：每句话都获得同一份带 `sourceId` 的局部语义事实和受本地门禁筛选的合法 choice；模型先判断它只是问答，还是确实让人物按 Soul、人格、身体、关系和记忆考虑或定下下一步。纯问题不得生成行动，只有 `accept + choice` 才进入行动链。服务端当场校验紧急生存、必须回应、履约与 follow-up，并保存稳定语义键；下一次可行动月份只在最新候选中本地唯一重配，不再调用第二轮模型猜自由文本。命中后才形成带 `sourceInteractionId` 的 DecisionFact / Intent；条件暂不允许时显示暂缓，候选消失或匹配不唯一时显示具体原因。人物卡继续读取真实 Intent 与 ActionFact，把后来定下、开始、被打断、做成或停下投影回原对话。
