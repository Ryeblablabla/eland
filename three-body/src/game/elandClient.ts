...
async function post<T>(route: string, body: unknown): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(detail.error ?? `${route} 返回 ${res.status}`);
  }
...
export const elandClient = {
  begin: (runId: string, civilizationId: number, skySample: SkySample, characterIds?: string[]) =>
    post<Frame>('begin', { runId, civilizationId, skySample, ...(characterIds && characterIds.length > 0 ? { characterIds } : {}) }),
  step: (runId: string, skySample: SkySample) => post<Frame | null>('step', { runId, skySample }),
  state: (runId: string) => get<{ playing: boolean; model: ModelProvider; frame: Frame | null }>(`state?runId=${encodeURIComponent(runId)}`),
